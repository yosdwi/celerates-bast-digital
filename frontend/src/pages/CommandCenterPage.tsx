  ...
  return (
    <div className="summary-strip" aria-label="Command Center summary">
      <div className="summary-item">
        <div className="summary-label">BAST ready</div>
        <div className="summary-value">{summary.bast_ready} / {summary.active_talents}</div>
        <div className="summary-meta">{readinessPercent(summary.bast_ready, summary.active_talents)}</div>
      </div>
      <div className="summary-item">
        <div className="summary-label">Need attention</div>
        <div className="summary-value">{summary.need_attention}</div>
        <div className="summary-meta">Current readiness rules</div>
      </div>
      <div className="summary-item">
        <div className="summary-label">Open tasks</div>
        <div className="summary-value">{summary.open_tasks}</div>
        <div className="summary-meta">Non-Closed in period</div>
      </div>
      <div className="summary-item">
        <div className="summary-label">Evidence ready</div>
        <div className="summary-value">{summary.evidence_ready} / {summary.active_talents}</div>
        <div className="summary-meta">{readinessPercent(summary.evidence_ready, summary.active_talents)}</div>
      </div>
    </div>
  );
}
...
export default function CommandCenterPage({ session, data, refreshing, onRefresh }: Props) {
  ...
  const readiness = useMemo(
    () => data.readiness.filter((item) => {
      const filterOk = readinessFilter === "all"
        || (readinessFilter === "attention" ? item.overall_state !== "complete" : item.role === readinessFilter);
      const searchOk = !normalizedSearch || `${item.name} ${item.nrp} ${item.role}`.toLocaleLowerCase().includes(normalizedSearch);
      ...
      <aside className={`sidebar ${sidebarOpen ? "open" : ""}`} aria-label="Workspace navigation">
        <div className="nav-group">
          ...
          {NAV_ITEMS.map(({ label, icon: Icon, active }) => (
            <button key={label} className={`nav-item ${active ? "active" : ""}`} type="button" disabled={!active} title={active ? undefined : "Available in a later slice"}>
              <Icon className="nav-icon" />
              <span>{label}</span>
              {label === "Actions" && data.summary.need_attention > 0 ? <span className="nav-count">{data.summary.need_attention}</span> : null}
            </button>
          ))}
        </div>
        <div className="nav-group">
      ...
      <main className="main-area">
        <header className="topbar">
          ...
          <div className={`global-search ${searchOpen ? "open" : ""}`}>
            ...
            <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search talents" aria-label="Search talents" />
          </div>
        ...
        <div className="content">
          <div className="page-heading">
            <div><h1>Command Center</h1><p>{data.period.label}</p></div>
            <button className="secondary-button refresh-button" type="button" disabled={refreshing} onClick={onRefresh}><RefreshIcon />{refreshing ? "Refreshing" : "Refresh"}</button>
          </div>
          ...
          <div className="ai-insight-strip">
            ...
            <div><strong>{insight.split(" · ")[0]}</strong>{insight.includes(" · ") ? ` · ${insight.split(" · ").slice(1).join(" · ")}` : ""}</div>
            <button type="button" onClick={() => setAiOpen(true)}>View insight</button>
          </div>
          ...
          <div className="top-grid">
            <section className="panel attention-panel">
              ...
              <div className="toolbar">
                <div className="panel-search"><SearchIcon /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search talent or NRP" aria-label="Search Need Attention" /></div>
                <select value={teamFilter} onChange={(event) => setTeamFilter(event.target.value as TeamFilter)} aria-label="Filter attention by team">
              ...
              <div className="desktop-table-wrap">
                <table className="data-table"><thead><tr><th>State</th><th>Talent</th><th>Primary blocker</th><th>Issues</th><th aria-label="Open" /></tr></thead><tbody>
                  {attention.map((item) => <tr key={item.employee_id} onClick={() => setSelected(item)}><td><StatusBadge state={item.overall_state} compact /></td><td><div className="talent-name">{item.name}</div><div className="cell-muted">{item.nrp} · {item.role}</div></td><td><div>{primaryBlocker(item)}</div><div className="cell-muted issue-preview">{issuePreview(item)}</div></td><td>{totalIssues(item)}</td><td><button className="row-open" type="button" aria-label={`Open ${item.name}`}><ChevronIcon /></button></td></tr>)}
                </tbody></table>
              </div>
              <div className="mobile-operational-list">
                {attention.map((item) => <button className="mobile-attention-row" type="button" key={item.employee_id} onClick={() => setSelected(item)}><div className="mobile-attention-top"><StatusBadge state={item.overall_state} compact /><span>{totalIssues(item)} issues</span></div><div className="mobile-attention-name">{item.name}</div><div className="mobile-attention-issue">{primaryBlocker(item)} · {issuePreview(item)}</div><div className="mobile-attention-role">{item.nrp} · {item.role}</div><ChevronIcon className="mobile-chevron" /></button>)}
              </div>
            </section>

            <section className="panel sources-panel">
              <div className="panel-title-row"><div><h2>Sources</h2><span>Last successful server ingest</span></div></div>
              <div className="source-list">
                {data.sources.map((source) => <div className="source-row" key={source.source_key}><span className={`source-dot ${source.last_success_at ? "observed" : "unknown"}`} /><div><strong>{source.label}</strong><span>{source.last_success_at ? new Date(source.last_success_at).toLocaleString() : "No successful ingest observed"}</span></div><div className="source-age">{sourceAge(source)}</div></div>)}
              </div>
              <div className="source-note">Freshness is shown as observed age only. No SLA threshold is inferred.</div>
            </section>
          </div>
          ...
          <section className="panel readiness-panel">
            ...
            <div className="filter-chips" role="group" aria-label="Talent readiness filters">
              {([['all','All'],['Developer','Developer'],['IoT Operations','IoT Operations'],['attention','Attention only']] as const).map(([value,label]) => <button key={value} type="button" className={readinessFilter === value ? "active" : ""} onClick={() => setReadinessFilter(value)}>{label}</button>)}
            </div>
            {readiness.length === 0 ? <div className="empty-state">No talents match this filter.</div> : null}
            <div className="desktop-table-wrap"><table className="data-table readiness-table"><thead><tr><th>Talent</th><th>Attendance</th><th>Timesheet</th><th>Task</th><th>Evidence</th><th>Overall</th></tr></thead><tbody>
              {readiness.map((item) => <tr key={item.employee_id}><td><div className="talent-name">{item.name}</div><div className="cell-muted">{item.nrp} · {item.role}</div></td><td><StateCell state={item.checks.attendance.state} /></td><td><StateCell state={item.checks.timesheet.state} /></td><td><StateCell state={item.checks.task.state} /></td><td><StateCell state={item.checks.evidence.state} /></td><td><StateCell state={item.overall_state} /></td></tr>)}
            </tbody></table></div>
            <div className="mobile-operational-list readiness-mobile-list">
              {readiness.map((item) => <div className="mobile-readiness-row" key={item.employee_id}><div className="mobile-readiness-head"><div><strong>{item.name}</strong><span>{item.nrp} · {item.role}</span></div><StatusBadge state={item.overall_state} compact /></div><div className="mobile-readiness-grid">{([['Attendance',item.checks.attendance],['Timesheet',item.checks.timesheet],['Task',item.checks.task],['Evidence',item.checks.evidence]] as const).map(([label,check]) => <div key={label}><span>{label}</span><strong className={`text-${check.state}`}>{statusLabel(check.state)}</strong></div>)}</div></div>)}
            </div>
          </section>
