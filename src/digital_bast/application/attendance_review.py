...
class AttendanceReviewEvidenceMetadata:
    request_id: UUID
    evidence_id: UUID
    content_type: str
    byte_size: int
...
@dataclass(frozen=True, slots=True)
class AttendanceReviewEvidence(AttendanceReviewEvidenceMetadata):
    content: bytes
...
class AttendanceReviewService:
    ...
    async def metadata(self, request_id: UUID) -> AttendanceReviewEvidenceMetadata | None:
        return await run_sync(self._metadata, request_id)

    async def evidence(self, request_id: UUID) -> AttendanceReviewEvidence | None:
        return await run_sync(self._evidence, request_id)

    def _metadata(self, request_id: UUID) -> AttendanceReviewEvidenceMetadata | None:
        try:
            ...
            raise InfrastructureError(
                service="postgres",
                operation="attendance_review_evidence_metadata",
            ) from error
        if row is None:
            return None
